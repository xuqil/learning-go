...
func Test_route_addRoute(t *testing.T) {
	...
	//	通配符和参数路由冲突
	r.addRoute(http.MethodGet, "/m/*/o", mockHandler)
	assert.PanicsWithValue(t, "route: 非法路由，已有通配符路由，不能再注册参数路由 [:id]", func() {
		r.addRoute(http.MethodGet, "/m/:id/o", mockHandler)
	})

	r.addRoute(http.MethodGet, "/i/:id/k", mockHandler)
	assert.PanicsWithValue(t, "route: 非法路由，已有参数路由，不能再注册通配符路由 [*]", func() {
		r.addRoute(http.MethodGet, "/i/*/k", mockHandler)
	})

	//	参数路由冲突
	r.addRoute(http.MethodGet, "/x/:name/z", mockHandler)
	assert.PanicsWithValue(t, "route: 非法路由，参数路由冲突 [:id]", func() {
		r.addRoute(http.MethodGet, "/x/:id/z", mockHandler)
	})
}
