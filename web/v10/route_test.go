...
func Test_route_addRoute(t *testing.T) {
	...
	// 非法正则表达式
	assert.PanicsWithValue(t, "route: 非法路由，正则表达式不能解析  ([0-9]+))", func() {
		r.addRoute(http.MethodGet, "/res/:id([0-9]+))", mockHandler)
	})
}
...
/*
BenchmarkRoute_findRoute_static

goos: linux
goarch: amd64
pkg: leanring-go/web
cpu: AMD Ryzen 7 5800H with Radeon Graphics
BenchmarkRoute_findRoute_star
BenchmarkRoute_findRoute_star-16        12106687                98.08 ns/op
*/
func BenchmarkRoute_findRoute_static(b *testing.B) {
	r := newRouter()
	var mockHandler HandleFunc = func(ctx *Context) {}
	r.addRoute(http.MethodGet, "/user/111/detail", mockHandler)

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		r.findRoute(http.MethodGet, "/user/111/detail")
	}
}
...
cpu: AMD Ryzen 7 5800H with Radeon Graphics
BenchmarkRoute_findRoute_star
BenchmarkRoute_findRoute_star-16        12005089                98.42 ns/op
*/
func BenchmarkRoute_findRoute_star(b *testing.B) {
	r := newRouter()
	var mockHandler HandleFunc = func(ctx *Context) {}
...
/*
BenchmarkRoute_findRoute_param
...
cpu: AMD Ryzen 7 5800H with Radeon Graphics
BenchmarkRoute_findRoute_param
BenchmarkRoute_findRoute_param-16        5492724               214.6 ns/op
*/
func BenchmarkRoute_findRoute_param(b *testing.B) {
	r := newRouter()
	var mockHandler HandleFunc = func(ctx *Context) {}
	r.addRoute(http.MethodGet, "/user/:id/detail", mockHandler)
...
/*
BenchmarkRoute_findRoute_regex
...
cpu: AMD Ryzen 7 5800H with Radeon Graphics
BenchmarkRoute_findRoute_regex
BenchmarkRoute_findRoute_regex-16        3476650               334.8 ns/op
*/
func BenchmarkRoute_findRoute_regex(b *testing.B) {
	r := newRouter()
	var mockHandler HandleFunc = func(ctx *Context) {}
	r.addRoute(http.MethodGet, "/user/:id([0-9]+)/detail", mockHandler)
